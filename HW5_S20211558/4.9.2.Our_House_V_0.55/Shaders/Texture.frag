...
#define DARK_GRAY		vec4(0.2f, 0.2f, 0.2f, 1.0f)

	if (mipmap_level < 1.0f) 
		final_color =  mix(RED, GREEN, mipmap_level);
	else if (mipmap_level < 2.0f)
	   final_color =  mix(GREEN, BLUE, mipmap_level - 1.0f);
	else if (mipmap_level < 3.0f)
	   final_color =  mix(BLUE, CYAN, mipmap_level - 2.0f);
	else if (mipmap_level < 4.0f)
	   final_color =  mix(CYAN, MAGENTA, mipmap_level - 3.0f);
	else if (mipmap_level < 5.0f)
	   final_color =  mix(MAGENTA, YELLOW, mipmap_level - 4.0f);
	else if (mipmap_level < 6.0f)
	   final_color =  mix(YELLOW, WHITE, mipmap_level - 5.0f);
	else if (mipmap_level < 7.0f)
	   final_color =  mix(WHITE, DARK_GRAY, mipmap_level - 6.0f);
	else 
